...
describe('sanitizeInstrumentForWorklet', () => {
	it('includes pwm fields for preview playback', () => {
		...
		extended.timerPwmSidSyncDuty = 25;
		extended.timerPwmSidSyncSweepMin = 5;
		extended.timerPwmSidSyncSweep = 3;
		extended.timerPwmSidSyncAutomationTrigger = 'free';
		extended.timerPwmSidSyncReverseSweep = true;

		const sanitized = sanitizeInstrumentForWorklet(instrument);

		expect(sanitized.timerPwmSidSyncDuty).toBe(25);
		expect(sanitized.timerPwmSidSyncSweepMin).toBe(5);
		expect(sanitized.timerPwmSidSyncSweep).toBe(3);
		expect(sanitized.timerPwmSidSyncAutomationTrigger).toBe('free');
		expect(sanitized.timerPwmSidSyncReverseSweep).toBe(true);
